...
def main():
    ...
    while True:
        player1 = input(f"Player1, pick any number from 1 to {starting_coins - 1} to remove from the pile: ")
        try:
            player1 = int(player1)
            break
        except ValueError:
    ...
    while game_on:
        while True:
            player2 = input(f"Player2, pick a number from 1 to {min(2 * player1, remaining_coins)} "
                            f"to remove from the pile: ")
            try:
                player2 = int(player2)
                break
            except ValueError:
        ...
        while True:
            player1 = input(f"Player1, pick a number from 1 to {min(2 * player2, remaining_coins)} "
                            f"to remove from the pile: ")
            try:
                player1 = int(player1)
                break
